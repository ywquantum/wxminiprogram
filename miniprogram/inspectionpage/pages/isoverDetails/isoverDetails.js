import { requestToimgs, requestToken } from "../../../http/request";
...
Page({
    ...
    onLoad(options) {
        ...
        if (data.event_type == "1") {
            ...
            requestToken(
                `xunjian/history_get?id=${data.listId}`,
                "get",
                {}
            ).then((res) => {
                console.log(res);
                ...
                let start =
                    res.data.main_record.event_date_start &&
                    getTimesOtherType(res.data.main_record.event_date_start);
                let end =
                    res.data.main_record.event_end_actual &&
                    getTimesOtherType(res.data.main_record.event_end_actual);
                ...
                requestToken(`xunjian/insert_inspection_track`, "get", {}).then(
                    (src) => {}
                );
            });
        } else {
